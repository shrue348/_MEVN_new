<template>
  <div class="login">

    <div class="login_wrap">
      <!-- <div class="login_logo" style="color: #45566B; font-size: 70px; margin-bottom: .4em; ">ЦРМ</div> -->
      <div class="login_logo">
        <img src="data:image/png;base64," alt="CRM">
      </div>

      <form class="login_form" onsubmit="return false" v-if="checkadmin && true">
        <div class="form-group-material">
          <input type="text" required v-model="form.login">
          <div class="form-group-material-highlight"></div>
          <label>Логин</label>
        </div>

        <div class="form-group-material">
          <input type="password" required v-model="form.password">
          <div class="form-group-material-highlight"></div>
          <label>Пароль</label>
        </div>

        <div class="form-group-material">
          <button class="login_btn btn" @click="login" @keyup:enter="login" :disabled="form.login.length == 0 || form.password.length == 0">
            <font-awesome-icon icon="sign-in-alt" class="mgr10"></font-awesome-icon> Войти
          </button>
        </div>
      </form>


      <form class="login_form" onsubmit="return false" v-if="!checkadmin && showForm">
        <div class="form-group-material">
          В системе отсутсвует аккаунт администратора (admin). Зарегистрируйте аккаунт администратора чтобы продолжить.
        </div>

        <div class="form-group-material">
          <input type="email" required v-model="form.adminEmail" name="create_admin__email">
          <div class="form-group-material-highlight"></div>
          <label>Email</label>
        </div>

        <div class="form-group-material">
          <input type="password" required v-model="form.adminPassword" @keyup:enter="createAmin" name="create_admin__password">
          <div class="form-group-material-highlight"></div>
          <label>Пароль</label>
        </div>

        <div class="form-group-material">
          <button class="login_btn btn" @click="createAmin" :disabled="form.adminPassword.length == 0 || form.adminEmail.length == 0">
            <font-awesome-icon icon="fire" class="mgr10"></font-awesome-icon> Создать
          </button>
        </div>
      </form>
    </div>

    <div class="login_copy">
      &copy; shure348 2018
    </div>

  </div>
</template>

<script>
import host from '../data/host.js'

export default {
  name: 'Login',
  data() {
    return {
      checkadmin: false,
      showForm: false,
      form: {
        login: '',
        password: '',
        adminPassword: '',
        adminEmail: ''
      }
    }
  },

  computed: {
    token() {
      return localStorage.getItem('token')
    }
  },

  methods: {
    login(){
      this.$http.post(`${host.host}/login`, {
        displayName: this.form.login,
        password: this.form.password
      }, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json'
        }
      }).then(response => {
        if (response.body == 'Login failed') return this.$toastr('error', 'Неправильный логин или пароль')
        localStorage.setItem('token', response.body.token)
        return this.$router.push('/tasks')
      }, response => {

        // return this.$toastr('error', data.message)
      })
    },

    createAmin(){
      this.$http.post(`${host.host}/user/createAdmin`, {
        displayName: 'admin',
        name: 'Администратор',
        email: this.form.adminEmail,
        password: this.form.adminPassword,
        position: 'Программист',
        active: true,
        role: 'admin'
      }, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json'
        }
      }).then(response => {
        this.checkAdmin()
        
      }, response => {

        return this.$toastr('error', 'Ошибка сервера. Аккаунт администратора не создан.')
      })
    },

    checkAdmin(){
      this.$http.get(`${host.host}/login/checkadmin`, {
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json'
        }
      }).then(response => {
        // let data = JSON.parse(response.data)
        this.showForm = true
        this.checkadmin = JSON.parse(response.body)
      }, response => {
        this.showForm = true
        // return this.$toastr('error', data.message)
      })
    }
  },

  created() {
    document.title = 'CRM | Вход'
    this.checkAdmin()

    if (this.token != null) return this.$router.push('/tasks')
  },

  directives: {
    focus: { //автофокус по v-focus
      inserted: function (el) {
        el.focus()
      }
    }
  }
}
</script>

<style lang="scss" scoped>
.login {
  width: 100vw;
  height: 100vh;
  display: flex;
  align-items: center;
  justify-content: center;
  padding: .5em;
  background-color: #ECEFF1;
  box-sizing: border-box;
  flex-wrap: wrap;
  flex-direction: column;

  &_wrap {
    display: flex;
    flex-direction: column;
    margin-top: auto;
    margin-bottom: auto;
    align-items: center;
  }

  &_logo {
    text-align: center;
    margin-bottom: 4.2em;
  }

  &_form {
    width: 280px;
  }

  &_btn {
    width: 100%;
  }

  &_copy {
    font-size: .8em;
    text-align: center;
  }
}

input { box-shadow: 0 0 0 50px #ECEFF1 inset; }
</style>